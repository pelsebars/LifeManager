...
import type { BacklogItem, BacklogBucket, TaskCategory } from '../../types';
...
export function BacklogView() {
  const { items, buckets, loading, error, fetchAll, createItem, updateItem, deleteItem, createBucket, updateBucket, deleteBucket } = useBacklogStore();
  const { projects, phases, tasks, createTask } = usePlanningStore();

  const [showItemModal,   setShowItemModal]   = useState<{ open: boolean; item?: BacklogItem; defaultBucketId?: string | null }>({ open: false });
  ...
  // Build task option groups for the dependency picker in TaskDetailPanel (BL-46)
  ...
  const handleSaveBucket = async (name: string) => {
    if (showBucketModal.bucket) {
      await updateBucket(showBucketModal.bucket.id, name);
    } else {
  ...
  // BL-46: create the real task, then remove from backlog
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, sans-serif', background: '#f8fafc' }}>
      ...
      {/* Modals */}
      {showItemModal.open && (
        <ItemModal buckets={buckets} item={showItemModal.item} defaultBucketId={showItemModal.defaultBucketId}
          onSave={handleSaveItem} onClose={() => setShowItemModal({ open: false })} />
      )}
